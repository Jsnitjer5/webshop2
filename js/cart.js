    ...
    cartItem.innerHTML = `
      ...
      <div class="flex items-center">
        <div class="flex items-center border rounded overflow-hidden">
          <button class="decrease-quantity px-3 py-1 text-lg text-gray-600 hover:bg-gray-100" data-index="${index}">-</button>
          <span class="px-3 py-1 border-l border-r">${item.quantity}</span>
          <button class="increase-quantity px-3 py-1 text-lg text-gray-600 hover:bg-gray-100" data-index="${index}">+</button>
        </div>
        <span class="ml-4 text-lg font-medium w-24 text-right">€${itemTotal.toFixed(2)}</span>
        <button class="remove-item ml-4 p-1 text-red-500 hover:text-red-700" data-index="${index}">
  ...
  if (cartSummary) {
    ...
    cartSummary.innerHTML = `
      ...
      <div class="flex justify-between mb-2">
        ...
        <span class="${shipping === 0 ? 'text-green-600' : 'text-gray-900'}">${shipping === 0 ? 'Gratis' : '€' + shipping.toFixed(2)}</span>
      </div>
      <div class="border-t pt-2 mt-2">
        ...
        ${shipping === 0 ? 
          ...
          `<p class="mt-1 text-xs text-gray-500">Nog €${(50 - total).toFixed(2)} tot gratis verzending</p>`
        }
      </div>
    `;
  }
