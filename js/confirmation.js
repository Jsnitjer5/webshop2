...
function displayOrderConfirmation() {
  ...
  document.getElementById('shipping-address').textContent = `${order.customer.address}, ${order.customer.postalCode} ${order.customer.city}`;
  ...
  order.items.forEach(item => {
    ...
    itemElement.innerHTML = `
      ...
      <div class="ml-4 text-base font-medium text-gray-900">€${(item.price * item.quantity).toFixed(2)}</div>
    `;
  ...
  document.getElementById('order-shipping').textContent = order.shipping === 0 ? 'Gratis' : `€${order.shipping.toFixed(2)}`;
